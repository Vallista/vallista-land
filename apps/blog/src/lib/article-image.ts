...
//  - "assets/splash.jpg" 또는 "./assets/splash.jpg" → /contents/articles/<entryId>/assets/splash.jpg
//  - 나머지 → 그대로 (폴백)
export function normalizeArticleImage(value: string | undefined, entryId: string): string | undefined {
  ...
  if (trimmed.startsWith('assets/') || trimmed.startsWith('./assets/')) {
    ...
    return `/contents/articles/${segments}/assets/${stripped.split('/').map(encodeURIComponent).join('/')}`
  }
